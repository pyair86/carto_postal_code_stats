...
conf_json_reader = \
    ConfJsonReader('/myApp/App/config_files/flask_config.json')
flask_config = conf_json_reader.get_config('config')
app = Flask(__name__)
app.config.from_mapping(flask_config)
cache = Cache(app)
conf_json_reader = \
    ConfJsonReader('/myApp/App/config_files/db_credentials_config.json')
    ...
    try:
        ...
        if request.method == 'POST' and form.validate():
            RegisterHandler(form,
                            db_connection.get_cursor()).register_user()
...
def get_postal_code_stats(postal_code):
    ...
    try:
        ...
        stats_polygon_aggregator = \
            StatsPolygonAggregator(db_connection.get_cursor(),
                                   get_geojson_single_polygon_query,
                                   get_aggregated_turnover_single_polygon_query,
                                   postal_code)
        geojson = \
            stats_polygon_aggregator.get_geojson_aggregated_stats()
        if geojson:
...
@app.route('/postal_codes_agg/<string:postal_code>')
@cache.cached()
@is_logged_in
def get_postal_code_stats_agg(postal_code):
...
@app.route('/postal_codes_agg/')
@cache.cached()
@is_logged_in
def collect_features_agg():
    db_connection = None
